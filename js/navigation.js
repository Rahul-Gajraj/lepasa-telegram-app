class Navigator {
  ...
  init(reffUrl) {
    ...
    this.#controller.getLandingPageInfo(reffUrl, function (controllerData) {
      ...
      that.#controller.getLeagueInfo(function (controllerData) {
        that.setLeagueInfoOnPlayPage(controllerData);
      });
      ...
      setInterval(function () {
        ...
        if (that.#mineSyncCounter > 50) that.#mineSyncCounter = 0;
      }, 1000);
    });
  ...
  resetUserInfo() {
    ...
    this.#controller.getLandingPageInfo(null, function (controllerData) {
      that.#controller.getLeagueInfo(function (controllerDataLeague) {
        that.setLeagueInfoOnPlayPage(controllerDataLeague);
      });
    });
  }
  #hideAll() {
  ...
  landingPage(controllerData) {
    ...
    setTimeout(function () {
      if (controllerData && controllerData.isFreshUser === 1) {
        $("#txtWelcome_Bonus_Level_Text").text(
          controllerData.telegramLoyalty.name
        );
        $("#txtWelcome_Bonus_Level_Number").text(
          controllerData.telegramLoyalty.level
        );
        $("#txtWelcomeRewardNumber").text(
          controllerData.telegramLoyalty.amount
        );
        $("#btnFreshUserClaim")
          .off("click")
          .on("click", function () {
            that.#controller.updateWelcomeClaim(function (controllerData) {
              that.gotoPlayPage();
            });
          });
        that.gotoWelcomePage();
      } else that.gotoPlayPage();
    }, 300);
  }
  gotoWelcomePage() {
    $(".loading_screen").addClass("hide");
  ...
  openAutomatedTellerDrawer(vestingAmt) {
    ...
    $("#txtVestingAmount").text(
      vestingAmt.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
    );
    var that = this;
  ...
  #updatePlayePageUi() {
    ...
    $("#txtActiveBalance").text(
      userInfoData.balance.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
    );
    $("#txtBarCapacityRate").text(userInfoData.capacityRate);
    $("#progressMainPlay").css(
      "width",
      this.#controller.getEnergyValueInPercent() + "%"
    );
    var energyValue = this.#controller.getEnergyValue();
    $("#txtFillCapacity").text(energyValue);
    if (energyValue > 0)
      $("#miningAnimationValue")
        .removeClass("hide")
        .find(".animated-text")
        .text(
          "+" +
            (energyValue > userInfoData.miningRate
              ? userInfoData.miningRate
              : energyValue)
        );
    else $("#miningAnimationValue").addClass("hide");
        ...
        var html = `
                <li class="ref_item">
                    <div class="ref_item_container">
                        ...
                        ${
                          (dataRow.claimed === false &&
                          dataRow.current >= dataRow.max
                            ? `<button type="button" class="claim" id="btnReferralClaim_${i}">Claim</button>`
                            : "") +
                          (dataRow.claimed === true
                            ? `<span>Claimed</span>`
                            : "")
                        }                        
                    </div>
  ...
  gotoEarnPage(willRemainAtRewards) {
    ...
    $("#tabCommunity,#tabDailyTask,#tabRewards")
      .off("click.tabnav")
      .on("click.tabnav", function () {
        that.#helper.activateTab(this);
      });
  ...
  shareReferral() {
    window.open(
      "https://t.me/share/url?url=" +
        this.#controller.getUserInfoData().referralLink,
      "_blank"
    );
  }
  ...
  setLeagueInfoOnPlayPage(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i].claimed === true) currentLeague = data[i].name;
    }
    $("#currentLeague_Name").text(currentLeague);
    $("#currentLeague_Img").attr(
      "src",
      `/public/leagues/${currentLeague.toLowerCase()}.png`
    );
  }
  gotoLeagueInfoPage() {
    this.#hideAll();
      ...
      var html = `
                <div class="leagues_item">
                    <div class="leagues_item_container">
                        ...
                        ${
                          leagueInfo.claimed === false &&
                          leagueInfo.status === true
                            ? `<button class="claim" id="btnOpenLeagueClaimDrawer_${i}">Claim</button>`
                            : leagueInfo.claimed === true &&
                              leagueInfo.status === false
      ...
      $("#btnOpenLeagueClaimDrawer_" + i)
        ...
        .on("click", function () {
          ...
          if (leagueInfo.cards !== null) {
            ...
            $("#league_Claim_drawer .league_Claim_drawer_container")
              .empty()
              .append(
                `<img src="${leagueInfo.cards.image}" style="width:100%;" />`
              );
  ...
  #shopPopulatePageUi(dataset) {
    var that = this;
    if (
      dataset.boosterStatus.currentEnergyCount == 0 &&
      dataset.boosterStatus.currentGPUCount == 0
    ) {
    ...
    $("#btnOpenEnergyBarRefillModal .shop_item_content .shop_item_num").text(
      dataset.boosterStatus.currentEnergyCount +
        "/" +
        dataset.boosterStatus.energyDailyLimit
    );
    $("#btnOpenGPUBoosterModal .shop_item_content .shop_item_num").text(
      dataset.boosterStatus.currentGPUCount +
        "/" +
        dataset.boosterStatus.gpuDailyLimit
    );

    if (dataset.spinLevel.isNext === "1") {
      $("#btnOpen_BarRefillSpeed_LevelUpgrade .shop_level_item_stats").empty()
        .append(`
                <div class="shop_level_item_coins">
                    <img src="/public/coin.png" height="20px" width="20px" />
                    <p class="cost-text">${
                      dataset.spinLevel.nextUpgradeCost
                    }</p>
                </div>
                <p class="level-text">| ${
                  Number(dataset.spinLevel.id) + 1
                } level</p>`);
    } else {
      $("#btnOpen_BarRefillSpeed_LevelUpgrade .shop_level_item_stats")
        .empty()
        .append(`<p class="level-text">Maximum Level Reached</p>`);
    }

    if (dataset.capacityLevel.isNext === "1") {
      $(
        "#btnOpen_EnergyBarCapacity_LevelUpgrade .shop_level_item_stats"
      ).empty().append(`
                <div class="shop_level_item_coins">
                    <img src="/public/coin.png" height="20px" width="20px" />
                    <p class="cost-text">${
                      dataset.capacityLevel.nextUpgradeCost
                    }</p>
                </div>
                <p class="level-text">| ${
                  Number(dataset.capacityLevel.id) + 1
                } level</p>`);
    } else {
      $("#btnOpen_EnergyBarCapacity_LevelUpgrade .shop_level_item_stats")
        .empty()
    ...
    if (dataset.miningLevel.isNext === "1") {
      $("#btnOpen_GPUUpgrade_LevelUpgrade .shop_level_item_stats").empty()
        .append(`
                <div class="shop_level_item_coins">
                    <img src="/public/coin.png" height="20px" width="20px" />
                    <p class="cost-text">${
                      dataset.miningLevel.nextUpgradeCost
                    }</p>
                </div>
                <p class="level-text">| ${
                  Number(dataset.miningLevel.id) + 1
                } level</p>`);
    } else {
      $("#btnOpen_GPUUpgrade_LevelUpgrade .shop_level_item_stats")
        .empty()
        .append(`<p class="level-text">Maximum Level Reached</p>`);
    }

    ////Binding click events for booster update
    $("#btnOpenEnergyBarRefillModal")
      .data("upgradeListData", dataset)
      .off("click")
      .on("click", function () {
        ...
        $("#shop_booster_update_drawer .drawer_shop_heading").text(
          "Energy Bar Refill"
        );
        $("#shop_booster_update_drawer .drawer_shop_subheading").text(
          "Refill your spark tank to hundred persent."
        );
        $("#shop_booster_update_drawer .chip").text(
          dataset.boosterStatus.currentEnergyCount +
            "/" +
            dataset.boosterStatus.energyDailyLimit
        );
    ...
    $("#btnOpenGPUBoosterModal")
      ...
      .on("click", function () {
        ...
        $("#shop_booster_update_drawer .drawer_shop_subheading").text(
          "Convert all energy into points at once."
        );
        $("#shop_booster_update_drawer .chip").text(
          dataset.boosterStatus.currentGPUCount +
            "/" +
    ...
    $("#btnOpen_BarRefillSpeed_LevelUpgrade")
      .data("upgradeListData", dataset)
      .off("click")
      .on("click", function () {
        var dataset = $(this).data("upgradeListData");
        ...
        $("#shop_level_upgrade_drawer .drawer_shop_heading").text(
          "Bar Refill Speed"
        );
        $("#shop_level_upgrade_drawer .drawer_shop_subheading").text(
          "Increase Recharging Speed By"
        );
        $("#shop_level_upgrade_drawer .chip").html(
          Number(dataset.spinLevel.id) +
            1 +
            ' <img src="/public/spark_icon.svg" style="width:15px;margin-bottom:-7px;" /> / Per Spin'
        );
        $("#shop_level_upgrade_img").attr("src", "/public/shop_spinner.svg");
        that.#shopBindLevelUpgradeClickEvent("SPIN");
      });
    $("#btnOpen_EnergyBarCapacity_LevelUpgrade")
      .data("upgradeListData", dataset)
      .off("click")
      .on("click", function () {
        var dataset = $(this).data("upgradeListData");
        ...
        $("#shop_level_upgrade_drawer .chip").html(
          dataset.capacityLevel.nextGrowthValue +
            ' <img src="/public/spark_icon.svg" style="width:15px;margin-bottom:-7px;" />'
        );
        $("#shop_level_upgrade_img").attr("src", "/public/shop_energy_two.svg");
        that.#shopBindLevelUpgradeClickEvent("CAPACITY");
      });
    $("#btnOpen_GPUUpgrade_LevelUpgrade")
      .data("upgradeListData", dataset)
      .off("click")
      .on("click", function () {
        var dataset = $(this).data("upgradeListData");
        ...
        $("#shop_level_upgrade_drawer .drawer_shop_subheading").text(
          "Increase Pixel Rate By"
        );
        $("#shop_level_upgrade_drawer .chip").text(
          dataset.miningLevel.nextGrowthValue + "/ sec"
        );
  ...
  #shopBindBoosterUpdateClickEvent(type) {
    var that = this;
    $("#btnShopBoosterUpdate")
      .data("boosterUpdateType", type)
      .off("click")
      .on("click", function () {
  ...
  gotoMorePage() {
      ...
      for (var i = 0; i < controllerData.length; i++) {
        if (controllerData[i].cards !== null) {
          ...
          if (currentBalance < controllerData[i].endRange) {
            completionPercent =
              (currentBalance / controllerData[i].endRange) * 100;
          }
          var cardHtml = `
                    <div class="more_cards">
                        <h3 class="more_cards_heading">${
                          controllerData[i].cards.name
                        }</h3>
                        <div class="more_card_content">
                            ${
                              controllerData[i].claimed === false
                                ? '<img class="gpu_card_lock" src="/public/padlock.svg" height="85px" width="85px" />'
                                : ""
                            }
                            <img class="gpu_card_img" src="${
                              controllerData[i].cards.image
                            }" alt="card" />
                        </div>
                        <h3>${
                          currentBalance > controllerData[i].endRange
                            ? controllerData[i].endRange
                            : currentBalance
                        } / ${controllerData[i].endRange}</h3>
                        <div class="progress_bar_container">
    ...
    //// Bind open close tabs
    $("#more_card_tab,#more_boost_stake_tab,#more_info_tab")
      .off("click.tabnav")
      .on("click.tabnav", function () {
        that.#helper.activateTab(this);
      });
    ...
    $("#openMoreInfoLeaderboard_Referral_drawer")
      .off("click")
      .on("click", function () {
        that.hideAllDrawerContents();
        that.#helper.openDrawer();
    ...
    $("#chkLeaderboard_Referral_Dateflag")
      .off("change")
      .on("change", function () {
        that.#bindLeaderboardReferral();
      });
    $("#openMoreInfoLeaderboard_Player_drawer")
      .off("click")
      .on("click", function () {
        that.hideAllDrawerContents();
        that.#helper.openDrawer();
  ...
  #bindLeaderboardReferral() {
    ...
    subheader.empty();
    mainContainer.empty();
    that.#controller.getLeaderboardReferral(
      $("#chkLeaderboard_Referral_Dateflag").is(":checked"),
      function (controllerData) {
        if (controllerData.refferals.length > 0) {
          //////Bind sub header

          subheader.append(
            that.#bindMoreInfoLeaderboardCardHtml(
              controllerData.refferals[0],
              0,
              true
            )
          );
  ...
  #bindLeaderboardPlayer() {
    ...
    that.#controller.getLeaderboardPlayer(function (controllerData) {
      if (controllerData.users.length > 0) {
        //////Bind sub header

        subheader.append(
          that.#bindMoreInfoLeaderboardCardHtml(
            controllerData.users[0],
            0,
            false
          )
        );
    ...
    var html = `
                    ...
                    <div class="custom_card">
                        ...
                        <div class="user_info">
                            ...
                            <img src="/public/coin.png" height="20px" width="20px" />
                            <p class="referral_p user_referral_count">${(isForRefferal ===
                            true
                              ? data.referral_bonus
                              : data.points
                            )
                        ...
                        <p class="referral_p">#${
                          isForRefferal === true ? data.rank : data.user_rank
                        }</p>
                        <p class="referral_p ${
                          isForRefferal === true ? "" : "hide"
                        }">${data.referral_count}</p>
                        </div>
                    </div>
                    `;
    return html;
...
$(document).ready(function () {
  ...
  if (window.location.search && window.location.search !== "") {
    reffUrl = window.location.search
      .split("&")[0]
      .replace("?tgWebAppStartParam=", "");
  }
  var customNavigator = new Navigator();
  customNavigator.init(reffUrl);
  $("#btnGoToRefPage").click(function () {
    customNavigator.gotoRefPage();
  });
  $("#btnGoToPlayPage").click(function () {
    customNavigator.gotoPlayPage();
  });
  $("#btnShareReferral").click(function () {
    customNavigator.shareReferral();
  });
  $("#btnCopyRefUrl").click(function () {
    customNavigator.copyToClipboardRefUrl();
  });
  $("#btnGoToEarnPage").click(function () {
    customNavigator.gotoEarnPage();
  });
  ...
  $(".btn_go_to_league_info_page").click(function () {
    customNavigator.gotoLeagueInfoPage();
  });
  $(".leagues_close_icon").click(function () {
    customNavigator.gotoPlayPage();
  });
  $("#btnGoToShopPage").click(function () {
    customNavigator.gotoShopPage();
  });
  $("#btnGoToMorePage").click(function () {
    customNavigator.gotoMorePage();
  });
  $("#chkFidgetControl").change(function () {
    customNavigator.toggleFidgetControl();
  });
  $("#shop_refresh_btn").click(function () {
    customNavigator.gotoShopPage();
  });
  $("#daily_reward_refresh_btn").click(function () {
    customNavigator.gotoEarnPage(true);
  });
