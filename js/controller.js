class Controller {
    ...
    $.ajax({
      ...
      success: function (returnData) {
        if (returnData.status === true) {
          ...
          // return;
          if (successCallback) successCallback(that.#userInfoData);
  ...
  getReferralList(successCallback) {
    ...
    $.ajax({
      ...
      success: function (returnData) {
        if (returnData.status === true) {
          if (successCallback) successCallback(returnData.data);
        } else toast.show(returnData.error);
      },
      error: function (error) {
        that.#handleUnexpectedError(error);
  ...
  increaseEnergyValue() {
    ...
    if (currentEnergyVal > capacityRate) currentEnergyVal = capacityRate;
  ...
  mineEnergyValue(syncCounter) {
    ...
    if (energyValue > 0) {
      ...
      var miningEnergy =
        energyValue > energyToConsume ? energyToConsume : energyValue;
      var halfMiningRate = Math.ceil(miningRate / 2);
      var pointsGenerated =
        ((halfMiningRate + miningRate) / halfMiningRate) * miningEnergy;
  ...
  consumeAllEnergyValue() {
    ...
    if (energyToConsume > 0) {
      var miningRate = this.#userInfoData.miningRate;
      var pointsGenerated =
        energyToConsume * (Math.ceil(miningRate / 2) + miningRate);
      this.#energyValue = 0;
      this.#userInfoData.balance += pointsGenerated;
      this.#balanceToSync += pointsGenerated;
    }
  ...
  #handleUnexpectedError(error) {
    if (error.status === 401)
      toast.show(
        "Multiple login detacted or session expired. Please re-open app on this device."
      );
